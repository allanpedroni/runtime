...
namespace Otc.ComponentModel.DataAnnotations
{
    ...
    internal class ValidationAttributeStore
    {
        ...
        private TypeStoreItem GetTypeStoreItem(Type type)
        {
            ...
            lock (typeStoreItems)
            {
                if (!typeStoreItems.TryGetValue(type, out TypeStoreItem? item))
                {
        ...
        private class TypeStoreItem : StoreItem
        {
            ...
            internal PropertyStoreItem GetPropertyStoreItem(string propertyName)
            {
                ...
                {
                    throw new ArgumentException(SR.Format(SR.AttributeStore_Unknown_Property, type.Name, propertyName),
                                                nameof(propertyName));
                }
