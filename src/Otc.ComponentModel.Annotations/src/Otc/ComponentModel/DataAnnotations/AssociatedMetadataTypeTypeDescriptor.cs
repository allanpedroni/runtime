...
namespace Otc.ComponentModel.DataAnnotations
{
    internal class AssociatedMetadataTypeTypeDescriptor : CustomTypeDescriptor
    {
        ...
        private static class TypeDescriptorCache
        {
            ...
            private static readonly ConcurrentDictionary<(Type, string), Attribute[]> typeMemberCache = new ConcurrentDictionary<(Type, string), Attribute[]>();

            // Stores whether or not a type and associated metadata type has been checked for validity
            private static readonly ConcurrentDictionary<(Type, Type), bool> validatedMetadataTypeCache = new ConcurrentDictionary<(Type, Type), bool>();
            ...
            public static Type GetAssociatedMetadataType(Type type)
            {
                ...
                if (metadataTypeCache.TryGetValue(type, out associatedMetadataType))
                {
            ...
            public static Attribute[] GetAssociatedMetadata(Type type, string memberName)
            {
                ...
                if (typeMemberCache.TryGetValue(memberTuple, out attributes))
                {
                    return attributes;
                }
